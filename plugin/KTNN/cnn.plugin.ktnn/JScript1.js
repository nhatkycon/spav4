var tinfn = {
    urlDefault: adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.ktnn.Class1, cnn.plugin.ktnn',
    setup: function () {
    },
    loadgrid: function () {
        ...
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            ...
            { name: 'KTNN_Lang', width: 30, sortable: true, align: "center", hidden: true },
            { name: 'KTNN_ThuTu', width: 20, resizable: true, sortable: true, align: "center" },
            ...
            { name: 'KTNN_Active', width: 30, resizable: true, sortable: true, align: "center", formatter: 'checkbox' },
            { name: 'KTNN_Hot', width: 30, resizable: true, sortable: true, align: "center", formatter: 'checkbox' },
            { name: 'KTNN_HetHan', width: 30, resizable: true, sortable: true, align: "center", formatter: 'checkbox', hidden: true }
                    ...
                    sortorder: 'desc',
                    rowNum: 50,
                    rowList: [5,10,20, 50, 100, 200, 300],
                    multiselect: true,
                    multiboxonly: true,
                    pager: jQuery('#tinmdlktnn-Pagerql'),
                    ...
                    loadComplete: function () {
                        ...
                        //adm.regQS(searchTxt, 'tinmdlktnn-List');
...
//                            });
                        ...
                        adm.regQS($('.mdl-head-search-danhmuc'), 'tinmdlktnn-List');
                ...
                var searchTxt = $('.mdl-head-search-ktnn-ql');
                ...
                $('.mdl-head-search-ktnn-ql').keyup(function () {
                    tinfn.search();
                });
                ...
                var changeLangBtn = $('#Tintucdl-changeLangSlt');
    ...
    addLang: function () {
        ...
        var s = '';
        if (jQuery('#tinmdlktnn-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#tinmdlktnn-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                var treedata = $("#tinmdlktnn-List").jqGrid('getRowData', s);
                        ...
                        open: function () {
                            ...
                            $.ajax({
                                ...
                                success: function (dt) {
                                    ...
                                    var newDlg = $('#tinmdlktnn-dlgNew');
                                    //file upload
                                    var lbladm_uploadKTNN_fileLists = $('.adm-upload-fileLists', newDlg);
                                    //                                    var txtID = $('.ID', newDlg);
                                    //                                    $(txtID).val(data.ID);
                                    ...
                                    var txtNoiDungql_ktnn = $('.NoiDungql_ktnn', newDlg);

                                    $(txtNoiDungql_ktnn).val(data.NoiDung);

                                    var lblAnh = $('.Anh', newDlg);
                                    var lblAnhPrv = $('.adm-uploadKTNN-preview', newDlg);
    ...
    del: function (fn) {
        var s = '';
        s = jQuery("#tinmdlktnn-List").jqGrid('getGridParam', 'selarrrow').toString();
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa tin này?')) {
                ...
                for (j = 0; j < ss.length; j++) {
                    var treedata = $("#tinmdlktnn-List").jqGrid('getRowData', ss[j]);
            ...
            $.ajax({
                ...
                success: function (dt) {
                    jQuery("#tinmdlktnn-List").trigger('reloadGrid');
                }
            });
        }
    ...
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-ktnn-ql');
        var searchTxt = $('.mdl-head-filterdanhmuc');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin tức') {
            q = '';
        }
        var _Lang = $('#Tintucdl-changeLangSlt').val();
        var dm = $(searchTxt).attr('_value');
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#tinmdlktnn-List').jqGrid('setGridParam', { url: tinfn.urlDefault + "&subAct=get&q=" + q + "&DMID=" + dm + '&Lang=' + _Lang }).trigger("reloadGrid");
        }, 500);
    },
    save: function (validate, fn) {
        ...
        var txtNoiDungql_ktnn = $('.NoiDungql_ktnn', newDlg);
        var NoiDungql_ktnn = $(txtNoiDungql_ktnn).val();
...
//        }
    ...
    loadHtml: function (fn) {
        var dlg = $('#tinmdlktnn-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.ktnn.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
    ...
    addpopfn: function () {
        ...
        var ulpFn = function () {
            ...
            var uploadKTNNView = $('.adm-uploadKTNN-preview-img', newDlg);
        ...
        var _ID = ID.val();
        adm.regType(typeof (danhmuc), 'docsoft.plugin.danhmuc.Class1, docsoft.plugin.danhmuc', function () {
            danhmuc.autoCompleteLangBased(_ID, "KT_NN", txtDMID, function (event, ui) {
        ...
        //        adm.regType(typeof (danhmuc), 'docsoft.plugin.danhmuc.Class1, docsoft.plugin.danhmuc', function () {
        //            danhmuc.autoCompleteByLoaiLang('TIN', txtDMID, function (event, ui) {
        //                $(txtDMID).attr('_value', ui.item.id);
        //            });
        //        });
        ...
        var txttextarea = $('.NoiDungql_ktnn', newDlg);
        adm.createTinyMce(txttextarea);
        //        setTimeout(function () {
        //            $(txtNoiDungql_ktnn).next().find('iframe').css({ 'height': '200px' });
        //        }, 10000);
