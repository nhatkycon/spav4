GiaoThuongDaduyetFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.giaoThuong.Daduyet.Class1, cnn.plugin.giaoThuong'; },
    setup: function () { },
    thongbao: function () {
    ...
    loadgrid: function () {
        ...
        $('#GiaoThuongDaduyetFnMdl-subMdl').tabs({ show: function (event, ui) {
            if (ui.index == 0 && !initialized[0]) {
                $('#GiaoThuongDaduyetFnMdl-subLangMdl-List').jqGrid({
                    url: GiaoThuongDaduyetFn.urlDefault().toString(),
                    datatype: 'json',
                    loadui: 'disable',
        ...
        var DMID = $('.mdl-head-search-GiaoThuongDaduyet');
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                $('#GiaoThuongDaduyetFnMdl-List').jqGrid({
                    url: GiaoThuongDaduyetFn.urlDefault().toString() + '&subAct=get&Lang=' + Lang,
                    datatype: 'json',
                    loadui: 'disable',
                    colNames: ['ID', 'LangBased', 'Lang', 'Ảnh', 'Tên', 'Mô tả', 'Danh mục', 'Xuất xứ', 'Ngày ĐK', 'Username', 'Publish', 'Active'],
                    colModel: [
                    ...
                    multiselect: true,
                    pager: $('#GiaoThuongDaduyetFnMdl-Pager'),
                    onSelectRow: function (rowid) {
                        GiaoThuongDaduyetFn.loadSubGrid(rowid);
                    },
                    loadComplete: function () {
                        adm.loading(null);
                        var txtfilter = $('.mdl-head-filterDanhMucGiaoThuongDaduyet', '#GiaoThuongDaduyetFnMdl-head');
                        var txtfilterTINH = $('.mdl-head-filterKhuVucGiaoThuongDaduyet', '#GiaoThuongDaduyetFnMdl-head');
                ...
                var filterLoaiDanhMucID = $('.mdl-head-filterDanhMucGiaoThuongDaduyet', '#GiaoThuongDaduyetFnMdl-head');
                var filterTINHID = $('.mdl-head-filterKhuVucGiaoThuongDaduyet', '#GiaoThuongDaduyetFnMdl-head');
                var searchTxt = $('.mdl-head-search-GiaoThuongDaduyet', '#GiaoThuongDaduyetFnMdl-head');
                ...
                var muaban = $('#GiaoThuongDaduyetFnMdl-muaban');
                ...
                var TrangThai = $('#GiaoThuongDaduyetFnMdl-TrangThai');
                ...
                $(TrangThai).attr('value',4);


                var changeLangBtn = $('#GiaoThuongDaduyetFnMdl-changeLangSlt');
    ...
    loadSubGrid: function (_id) {
        if (_id != null) {
            ...
            $('#GiaoThuongDaduyetFnMdl-subLangMdl-List').jqGrid('setGridParam', {
                url: GiaoThuongDaduyetFn.urlDefault().toString() + "&subAct=getByLangBasedId" + "&LangBased_ID=" + _id
                , loadComplete: function () {
                    adm.loading(null);
                }
            }).trigger("reloadGrid");
    ...
    filterfn: function (_active, _pub) {
        $('#GiaoThuongDaduyetFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongDaduyetFn.urlDefault().toString() + "&subAct=get" + "&Active=" + _active + "&Publish=" + _pub }).trigger("reloadGrid");
    },
    EditLang: function () {
        var s = '';
        if (jQuery('#GiaoThuongDaduyetFnMdl-subLangMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongDaduyetFnMdl-subLangMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                GiaoThuongDaduyetFn.loadHtml(function () {
                    var newDlg = $('#GiaoThuongDaduyetFnMdl-dlgNew');
                    GiaoThuongDaduyetFn.popfn();
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 750,
                        height: 550,
                        buttons: {
                            'Lưu': function () {
                                GiaoThuongDaduyetFn.save(false, function () {
                                    GiaoThuongDaduyetFn.clearform();
                                    jQuery('#GiaoThuongDaduyetFnMdl-subLangMdl-List').trigger('reloadGrid');
                                });
                            },
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            $.ajax({
                                url: GiaoThuongDaduyetFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
...
//                                    if (dt.Hot4) { Hot4.attr('checked', 'checked'); } else { Hot4.removeAttr('checked'); }
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                if (confirm('Bạn có chắc chắn xóa danh mục này?')) {
                    $.ajax({
                        url: GiaoThuongDaduyetFn.urlDefault().toString() + '&subAct=del',
                        dataType: 'script',
                        data: { 'ID': s },
                        success: function (dt) {
                            jQuery('#GiaoThuongDaduyetFnMdl-subLangMdl-List').trigger('reloadGrid');
    ...
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-GiaoThuongDaduyet');
        var searchDM = $('.mdl-head-filterDanhMucGiaoThuongDaduyet');
        var searchTINH = $('.mdl-head-filterKhuVucGiaoThuongDaduyet');
        var _Lang = $('#GiaoThuongDaduyetFnMdl-changeLangSlt').val();
        var filtermuaban = $('#GiaoThuongDaduyetFnMdl-muaban');
        var filterTrangThai = $('#GiaoThuongDaduyetFnMdl-TrangThai');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin') {
            q = '';
        }
        ...
        timerSearch = setTimeout(function () {
            $('#GiaoThuongDaduyetFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongDaduyetFn.urlDefault().toString() + '&subAct=get&dm=' + dm + '&dmkv=' + tinh + '&Lang=' + _Lang + '&q=' + q + '&muaban=' + muaban + '&trangthai=' + trangthai }).trigger('reloadGrid');
        }, 500);
    },
    add: function () {
        GiaoThuongDaduyetFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        GiaoThuongDaduyetFn.save(false, function () {
                            GiaoThuongDaduyetFn.clearform();
                            jQuery('#GiaoThuongDaduyetFnMdl-List').trigger('reloadGrid');
                        });
                    },
                    'Lưu và đóng': function () {
                        GiaoThuongDaduyetFn.save(false, function () {
    ...
    loadHtml: function (fn) {
        var dlg = $('#GiaoThuongDaduyetFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.giaoThuong.Daduyet.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
    ...
    edit: function () {
        var s = '';
        if (jQuery('#GiaoThuongDaduyetFnMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongDaduyetFnMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            alert('Chọn mẩu tin để sửa');
        }
...
//        var _Hot4 = Hot4.is(':checked');
        ...
        if (_LangBased_ID != '' && eval(_LangBased) == false) {
            $.ajax({
                url: GiaoThuongDaduyetFn.urlDefault().toString() + '&subAct=CountLang',
                dataType: 'script',
                data: {
                    'ID': _LangBased_ID,
                    'Lang': _Lang
...
//                    'Hot4': _Hot4
    ...
    del: function () {
        var s = '';
        s = jQuery('#GiaoThuongDaduyetFnMdl-List').jqGrid('getGridParam', 'selarrrow').toString();
    ...
    AddLang: function () {
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                var treedata = $("#GiaoThuongDaduyetFnMdl-List").jqGrid('getRowData', s);
    ...
    PheDuyet: function (active) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn duyệt các mẩu tin này?')) {
                $.ajax({
                    url: GiaoThuongDaduyetFn.urlDefault().toString() + '&subAct=PheDuyet' + '&active='+ active,
                    dataType: 'script',
                    data: {
                        'ID': s,
                    },
