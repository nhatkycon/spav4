GiaoThuongNoiBatFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.giaoThuong.User.GiaoThuongNoiBat.Class1, cnn.plugin.giaoThuong'; },
    setup: function () { },
    thongbao: function () {
    ...
    loadgrid: function () {
        ...
        var DMID = $('.mdl-head-search-GiaoThuongNoiBat');
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                $('#GiaoThuongNoiBatFnMdl-List').jqGrid({
                    url: GiaoThuongNoiBatFn.urlDefault().toString() + '&subAct=get&Lang=' + Lang + '&trangthai=6',
                    datatype: 'json',
                    loadui: 'disable',
                    ...
                    multiselect: true,
                    pager: $('#GiaoThuongNoiBatFnMdl-Pager'),
                    loadComplete: function () {
                        adm.loading(null);
                        var txtfilter = $('.mdl-head-filterDanhMucGiaoThuongNoiBat', '#GiaoThuongNoiBatFnMdl-head');
                        var txtfilterTINH = $('.mdl-head-filterKhuVucGiaoThuongNoiBat', '#GiaoThuongNoiBatFnMdl-head');
                ...
                var filterLoaiDanhMucID = $('.mdl-head-filterDanhMucGiaoThuongNoiBat', '#GiaoThuongNoiBatFnMdl-head');
                var filterTINHID = $('.mdl-head-filterKhuVucGiaoThuongNoiBat', '#GiaoThuongNoiBatFnMdl-head');
                var searchTxt = $('.mdl-head-search-GiaoThuongNoiBat', '#GiaoThuongNoiBatFnMdl-head');
                ...
                var muaban = $('#GiaoThuongNoiBatFnMdl-muaban');
                ...
                var TrangThai = $('#GiaoThuongNoiBatFnMdl-TrangThai');
                $(TrangThai).find('option').remove();
                $(TrangThai).prepend('<option value="7">Tin nổi bật đã duyệt</option>');
                $(TrangThai).prepend('<option value="6">Tin nổi bật chờ duyệt</option>');
                $(TrangThai).prepend('<option value="8">Tin nổi bật hết hạn</option>');
                $(TrangThai).prepend('<option value="9">Tin nổi bật hết hạn</option>');
                $(TrangThai).attr('value',6);


                var changeLangBtn = $('#GiaoThuongNoiBatFnMdl-changeLangSlt');
    ...
    loadSubGrid: function (_id) {
        if (_id != null) {
            ...
            $('#GiaoThuongNoiBatFnMdl-subLangMdl-List').jqGrid('setGridParam', {
                url: GiaoThuongNoiBatFn.urlDefault().toString() + "&subAct=getByLangBasedId" + "&LangBased_ID=" + _id
                , loadComplete: function () {
                    adm.loading(null);
                }
            }).trigger("reloadGrid");
    ...
    filterfn: function (_active, _pub) {
        $('#GiaoThuongNoiBatFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongNoiBatFn.urlDefault().toString() + "&subAct=get" + "&Active=" + _active + "&Publish=" + _pub }).trigger("reloadGrid");
    },
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-GiaoThuongNoiBat');
        var searchDM = $('.mdl-head-filterDanhMucGiaoThuongNoiBat');
        var searchTINH = $('.mdl-head-filterKhuVucGiaoThuongNoiBat');
        var _Lang = $('#GiaoThuongNoiBatFnMdl-changeLangSlt').val();
        var filtermuaban = $('#GiaoThuongNoiBatFnMdl-muaban');
        var filterTrangThai = $('#GiaoThuongNoiBatFnMdl-TrangThai');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin') {
            q = '';
        }
        ...
        timerSearch = setTimeout(function () {
            $('#GiaoThuongNoiBatFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongNoiBatFn.urlDefault().toString() + '&subAct=get&dm=' + dm + '&dmkv=' + tinh + '&Lang=' + _Lang + '&q=' + q + '&muaban=' + muaban + '&trangthai=' + trangthai }).trigger('reloadGrid');
        }, 500);
    },
    add: function () {
        GiaoThuongNoiBatFn.loadHtml(function () {
            var newDlg = $('#GiaoThuongNoiBatFnMdl-dlgNew');
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        GiaoThuongNoiBatFn.save(false, function () {
                            GiaoThuongNoiBatFn.clearform();
                            jQuery('#GiaoThuongNoiBatFnMdl-List').trigger('reloadGrid');
                        });
                    },
    ...
    loadHtml: function (fn) {
        var dlg = $('#GiaoThuongNoiBatFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.giaoThuong.User.GiaoThuongNoiBat.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
    ...
    edit: function () {
        var s = '';
        if (jQuery('#GiaoThuongNoiBatFnMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongNoiBatFnMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
                ...
                GiaoThuongNoiBatFn.loadHtml(function () {
                    ...
                    $(newDlg).dialog({
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            $.ajax({
                                url: GiaoThuongNoiBatFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
...
//        var _Hot4 = Hot4.is(':checked');
        ...
        if (_LangBased_ID != '' && eval(_LangBased) == false) {
            $.ajax({
                url: GiaoThuongNoiBatFn.urlDefault().toString() + '&subAct=CountLang',
                dataType: 'script',
                data: {
                    'ID': _LangBased_ID,
                    'Lang': _Lang
                ...
                success: function (_dt) {
                    ...
                    $.ajax({
                        url: GiaoThuongNoiBatFn.urlDefault().toString(),
                        dataType: 'script',
                        type: 'POST',
                        data: {
                            'subAct': 'save',
...
//                    'Hot4': _Hot4
    ...
    del: function () {
        var s = '';
        s = jQuery('#GiaoThuongNoiBatFnMdl-List').jqGrid('getGridParam', 'selarrrow').toString();
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {
                $.ajax({
                    url: GiaoThuongNoiBatFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
                        jQuery('#GiaoThuongNoiBatFnMdl-List').trigger('reloadGrid');
    ...
    DKDVVIP: function () {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc đăng ký những tin này?')) {
                $.ajax({
                    url: GiaoThuongNoiBatFn.urlDefault().toString() + '&subAct=DKDVVIP',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
                        jQuery('#GiaoThuongNoiBatFnMdl-List').trigger('reloadGrid');
