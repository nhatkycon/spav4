GiaoThuongChaoBanFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.giaoThuong.ChaoBan.Class1, cnn.plugin.giaoThuong'; },
    setup: function () { },
    thongbao: function () {
    ...
    loadgrid: function () {
        ...
        $('#GiaoThuongChaoBanFnMdl-subMdl').tabs({ show: function (event, ui) {
            if (ui.index == 0 && !initialized[0]) {
                $('#GiaoThuongChaoBanFnMdl-subLangMdl-List').jqGrid({
                    url: GiaoThuongChaoBanFn.urlDefault().toString(),
                    datatype: 'json',
                    loadui: 'disable',
        ...
        var DMID = $('.mdl-head-search-GiaoThuongChaoBan');
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                $('#GiaoThuongChaoBanFnMdl-List').jqGrid({
                    url: GiaoThuongChaoBanFn.urlDefault().toString() + '&subAct=get&Lang=' + Lang,
                    datatype: 'json',
                    loadui: 'disable',
                    colNames: ['ID', 'LangBased', 'Lang', 'Ảnh', 'Tên', 'Mô tả', 'Danh mục', 'Xuất xứ', 'Ngày ĐK', 'Username', 'Publish', 'Active'],
                    colModel: [
                    ...
                    multiselect: true,
                    pager: $('#GiaoThuongChaoBanFnMdl-Pager'),
                    onSelectRow: function (rowid) {
                        GiaoThuongChaoBanFn.loadSubGrid(rowid);
                    },
                    loadComplete: function () {
                        adm.loading(null);
                        var txtfilter = $('.mdl-head-filterDanhMucGiaoThuongChaoBan', '#GiaoThuongChaoBanFnMdl-head');
                        var txtfilterTINH = $('.mdl-head-filterKhuVucGiaoThuongChaoBan', '#GiaoThuongChaoBanFnMdl-head');
                ...
                var filterLoaiDanhMucID = $('.mdl-head-filterDanhMucGiaoThuongChaoBan', '#GiaoThuongChaoBanFnMdl-head');
                var filterTINHID = $('.mdl-head-filterKhuVucGiaoThuongChaoBan', '#GiaoThuongChaoBanFnMdl-head');
                var searchTxt = $('.mdl-head-search-GiaoThuongChaoBan', '#GiaoThuongChaoBanFnMdl-head');
                ...
                var muaban = $('#GiaoThuongChaoBanFnMdl-muaban');
                ...
                var TrangThai = $('#GiaoThuongChaoBanFnMdl-TrangThai');
                ...
                var changeLangBtn = $('#GiaoThuongChaoBanFnMdl-changeLangSlt');
    ...
    loadSubGrid: function (_id) {
        if (_id != null) {
            ...
            $('#GiaoThuongChaoBanFnMdl-subLangMdl-List').jqGrid('setGridParam', {
                url: GiaoThuongChaoBanFn.urlDefault().toString() + "&subAct=getByLangBasedId" + "&LangBased_ID=" + _id
                , loadComplete: function () {
                    adm.loading(null);
                }
            }).trigger("reloadGrid");
    ...
    filterfn: function (_active, _pub) {
        $('#GiaoThuongChaoBanFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongChaoBanFn.urlDefault().toString() + "&subAct=get" + "&Active=" + _active + "&Publish=" + _pub }).trigger("reloadGrid");
    },
    EditLang: function () {
        var s = '';
        if (jQuery('#GiaoThuongChaoBanFnMdl-subLangMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongChaoBanFnMdl-subLangMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                GiaoThuongChaoBanFn.loadHtml(function () {
                    var newDlg = $('#GiaoThuongChaoBanFnMdl-dlgNew');
                    GiaoThuongChaoBanFn.popfn();
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 750,
                        height: 550,
                        buttons: {
                            'Lưu': function () {
                                GiaoThuongChaoBanFn.save(false, function () {
                                    GiaoThuongChaoBanFn.clearform();
                                    jQuery('#GiaoThuongChaoBanFnMdl-subLangMdl-List').trigger('reloadGrid');
                                });
                            },
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            $.ajax({
                                url: GiaoThuongChaoBanFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
...
//                                    if (dt.Hot4) { Hot4.attr('checked', 'checked'); } else { Hot4.removeAttr('checked'); }
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                if (confirm('Bạn có chắc chắn xóa danh mục này?')) {
                    $.ajax({
                        url: GiaoThuongChaoBanFn.urlDefault().toString() + '&subAct=del',
                        dataType: 'script',
                        data: { 'ID': s },
                        success: function (dt) {
                            jQuery('#GiaoThuongChaoBanFnMdl-subLangMdl-List').trigger('reloadGrid');
    ...
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-GiaoThuongChaoBan');
        var searchDM = $('.mdl-head-filterDanhMucGiaoThuongChaoBan');
        var searchTINH = $('.mdl-head-filterKhuVucGiaoThuongChaoBan');
        var _Lang = $('#GiaoThuongChaoBanFnMdl-changeLangSlt').val();
        var filtermuaban = $('#GiaoThuongChaoBanFnMdl-muaban');
        var filterTrangThai = $('#GiaoThuongChaoBanFnMdl-TrangThai');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin') {
            q = '';
        }
        ...
        timerSearch = setTimeout(function () {
            $('#GiaoThuongChaoBanFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongChaoBanFn.urlDefault().toString() + '&subAct=get&dm=' + dm + '&dmkv=' + tinh + '&Lang=' + _Lang + '&q=' + q + '&muaban=' + muaban + '&trangthai=' + trangthai }).trigger('reloadGrid');
        }, 500);
    },
    add: function () {
        GiaoThuongChaoBanFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        GiaoThuongChaoBanFn.save(false, function () {
                            GiaoThuongChaoBanFn.clearform();
                            jQuery('#GiaoThuongChaoBanFnMdl-List').trigger('reloadGrid');
                        });
                    },
                    'Lưu và đóng': function () {
                        GiaoThuongChaoBanFn.save(false, function () {
    ...
    loadHtml: function (fn) {
        var dlg = $('#GiaoThuongChaoBanFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.giaoThuong.ChaoBan.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
    ...
    edit: function () {
        var s = '';
        if (jQuery('#GiaoThuongChaoBanFnMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongChaoBanFnMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            alert('Chọn mẩu tin để sửa');
        }
...
//                                    if (dt.Hot4) { Hot4.attr('checked', 'checked'); } else { Hot4.removeAttr('checked'); }
    ...
    save: function (validate, fn) {
        ...
        if (_LangBased_ID != '' && eval(_LangBased) == false) {
            $.ajax({
                url: GiaoThuongChaoBanFn.urlDefault().toString() + '&subAct=CountLang',
                dataType: 'script',
                data: {
                    'ID': _LangBased_ID,
                    'Lang': _Lang
...
//                    'Hot4': _Hot4
    ...
    del: function () {
        var s = '';
        s = jQuery('#GiaoThuongChaoBanFnMdl-List').jqGrid('getGridParam', 'selarrrow').toString();
    ...
    AddLang: function () {
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                var treedata = $("#GiaoThuongChaoBanFnMdl-List").jqGrid('getRowData', s);
    ...
    PheDuyet: function (active) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn duyệt các mẩu tin này?')) {
                $.ajax({
                    url: GiaoThuongChaoBanFn.urlDefault().toString() + '&subAct=PheDuyet' + '&active='+ active,
                    dataType: 'script',
                    data: {
                        'ID': s,
                    },
