GiaoThuongDangKyNoiBatFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.giaoThuong.DangKyNoiBat.Class1, cnn.plugin.giaoThuong'; },
    setup: function () { },
    thongbao: function () {
    ...
    loadgrid: function () {
        ...
        var DMID = $('.mdl-head-search-GiaoThuongDangKyNoiBat');
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                $('#GiaoThuongDangKyNoiBatFnMdl-List').jqGrid({
                    url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + '&subAct=get&Lang=' + Lang+'&trangthai=6',
                    datatype: 'json',
                    loadui: 'disable',
                    ...
                    multiselect: true,
                    pager: $('#GiaoThuongDangKyNoiBatFnMdl-Pager'),
                    onSelectRow: function (rowid) {
                        GiaoThuongDangKyNoiBatFn.loadSubGrid(rowid);
                    },
                    loadComplete: function () {
                        adm.loading(null);
                        var txtfilter = $('.mdl-head-filterDanhMucGiaoThuongDangKyNoiBat', '#GiaoThuongDangKyNoiBatFnMdl-head');
                        var txtfilterTINH = $('.mdl-head-filterKhuVucGiaoThuongDangKyNoiBat', '#GiaoThuongDangKyNoiBatFnMdl-head');
                        adm.regType(typeof (danhmuc), 'docsoft.plugin.danhmuc.Class1, docsoft.plugin.danhmuc', function () {
                            danhmuc.autoCompleteLangBased("", "SP_NHOM", txtfilter, function (event, ui) {
                                ...
                                GiaoThuongDangKyNoiBatFn.search();
                            });
                ...
                var filterLoaiDanhMucID = $('.mdl-head-filterDanhMucGiaoThuongDangKyNoiBat', '#GiaoThuongDangKyNoiBatFnMdl-head');
                var filterTINHID = $('.mdl-head-filterKhuVucGiaoThuongDangKyNoiBat', '#GiaoThuongDangKyNoiBatFnMdl-head');
                var searchTxt = $('.mdl-head-search-GiaoThuongDangKyNoiBat', '#GiaoThuongDangKyNoiBatFnMdl-head');
                ...
                var muaban = $('#GiaoThuongDangKyNoiBatFnMdl-muaban');
                ...
                var TrangThai = $('#GiaoThuongDangKyNoiBatFnMdl-TrangThai');
                $(TrangThai).find('option').remove();
                $(TrangThai).prepend('<option value="7">Đã duyệt</option>');
                $(TrangThai).prepend('<option value="6">Chờ duyệt</option>');
                $(TrangThai).prepend('<option value="8">Quá hạn</option>');
                $(TrangThai).prepend('<option value="9">Còn hạn</option>');
                $(TrangThai).attr('value',6);


                var changeLangBtn = $('#GiaoThuongDangKyNoiBatFnMdl-changeLangSlt');
    ...
    loadSubGrid: function (_id) {
        if (_id != null) {
            ...
            $('#GiaoThuongDangKyNoiBatFnMdl-subLangMdl-List').jqGrid('setGridParam', {
                url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + "&subAct=getByLangBasedId" + "&LangBased_ID=" + _id
                , loadComplete: function () {
                    adm.loading(null);
                }
            }).trigger("reloadGrid");
    ...
    filterfn: function (_active, _pub) {
        $('#GiaoThuongDangKyNoiBatFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + "&subAct=get" + "&Active=" + _active + "&Publish=" + _pub }).trigger("reloadGrid");
    },
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-GiaoThuongDangKyNoiBat');
        var searchDM = $('.mdl-head-filterDanhMucGiaoThuongDangKyNoiBat');
        var searchTINH = $('.mdl-head-filterKhuVucGiaoThuongDangKyNoiBat');
        var _Lang = $('#GiaoThuongDangKyNoiBatFnMdl-changeLangSlt').val();
        var filtermuaban = $('#GiaoThuongDangKyNoiBatFnMdl-muaban');
        var filterTrangThai = $('#GiaoThuongDangKyNoiBatFnMdl-TrangThai');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin') {
            q = '';
        }
        ...
        timerSearch = setTimeout(function () {
            $('#GiaoThuongDangKyNoiBatFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + '&subAct=get&dm=' + dm + '&dmkv=' + tinh + '&Lang=' + _Lang + '&q=' + q + '&muaban=' + muaban + '&trangthai=' + trangthai }).trigger('reloadGrid');
        }, 500);
    },
    add: function () {
        GiaoThuongDangKyNoiBatFn.loadHtml(function () {
            var newDlg = $('#GiaoThuongDangKyNoiBatFnMdl-dlgNew');
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        GiaoThuongDangKyNoiBatFn.save(false, function () {
                            GiaoThuongDangKyNoiBatFn.clearform();
                            jQuery('#GiaoThuongDangKyNoiBatFnMdl-List').trigger('reloadGrid');
                        });
                    },
                ...
                open: function () {
                    adm.styleButton();
                    GiaoThuongDangKyNoiBatFn.clearform();
                    GiaoThuongDangKyNoiBatFn.popfn();
    ...
    loadHtml: function (fn) {
        var dlg = $('#GiaoThuongDangKyNoiBatFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.giaoThuong.DangKyNoiBat.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
    ...
    edit: function () {
        var s = '';
        if (jQuery('#GiaoThuongDangKyNoiBatFnMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongDangKyNoiBatFnMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
                ...
                GiaoThuongDangKyNoiBatFn.loadHtml(function () {
                    ...
                    $(newDlg).dialog({
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            $.ajax({
                                url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
...
//        var _Hot4 = Hot4.is(':checked');
        ...
        if (_LangBased_ID != '' && eval(_LangBased) == false) {
            $.ajax({
                url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + '&subAct=CountLang',
                dataType: 'script',
                data: {
                    'ID': _LangBased_ID,
                    'Lang': _Lang
                ...
                success: function (_dt) {
                    ...
                    $.ajax({
                        url: GiaoThuongDangKyNoiBatFn.urlDefault().toString(),
                        dataType: 'script',
                        type: 'POST',
                        data: {
                            'subAct': 'save',
...
//                    'Hot4': _Hot4
    ...
    del: function () {
        var s = '';
        s = jQuery('#GiaoThuongDangKyNoiBatFnMdl-List').jqGrid('getGridParam', 'selarrrow').toString();
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {
                $.ajax({
                    url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
                        jQuery('#GiaoThuongDangKyNoiBatFnMdl-List').trigger('reloadGrid');
    ...
    PheDuyet: function (active) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn duyệt các mẩu tin này?')) {
                $.ajax({
                    url: GiaoThuongDangKyNoiBatFn.urlDefault().toString() + '&subAct=PheDuyet' + '&active='+ active,
                    dataType: 'script',
                    data: {
                        'ID': s,
                    },
