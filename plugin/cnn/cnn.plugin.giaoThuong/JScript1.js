GiaoThuongFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.giaoThuong.Class1, cnn.plugin.giaoThuong'; },
    setup: function () { },
    thongbao: function () {
    ...
    loadgrid: function () {
        ...
        $('#GiaoThuongFnMdl-subMdl').tabs({ show: function (event, ui) {
            if (ui.index == 0 && !initialized[0]) {
                $('#GiaoThuongFnMdl-subLangMdl-List').jqGrid({
                    url: GiaoThuongFn.urlDefault().toString(),
                    datatype: 'json',
                    loadui: 'disable',
        ...
        var DMID = $('.mdl-head-search-GiaoThuong');
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                $('#GiaoThuongFnMdl-List').jqGrid({
                    url: GiaoThuongFn.urlDefault().toString() + '&subAct=get&Lang=' + Lang,
                    datatype: 'json',
                    loadui: 'disable',
                    colNames: ['ID', 'LangBased', 'Lang', 'Ảnh', 'Tên', 'Mô tả', 'Danh mục', 'Xuất xứ', 'Ngày ĐK', 'Username', 'Publish', 'Active'],
                    colModel: [
                    ...
                    multiselect: true,
                    pager: $('#GiaoThuongFnMdl-Pager'),
                    onSelectRow: function (rowid) {
                        GiaoThuongFn.loadSubGrid(rowid);
                    },
                    loadComplete: function () {
                        adm.loading(null);
                        var txtfilter = $('.mdl-head-filterDanhMucGiaoThuong', '#GiaoThuongFnMdl-head');
                        var txtfilterTINH = $('.mdl-head-filterKhuVucGiaoThuong', '#GiaoThuongFnMdl-head');
                ...
                var filterLoaiDanhMucID = $('.mdl-head-filterDanhMucGiaoThuong', '#GiaoThuongFnMdl-head');
                var filterTINHID = $('.mdl-head-filterKhuVucGiaoThuong', '#GiaoThuongFnMdl-head');
                var searchTxt = $('.mdl-head-search-GiaoThuong', '#GiaoThuongFnMdl-head');
                ...
                var muaban = $('#GiaoThuongFnMdl-muaban');
                ...
                var TrangThai = $('#GiaoThuongFnMdl-TrangThai');
                ...
                var changeLangBtn = $('#GiaoThuongFnMdl-changeLangSlt');
    ...
    loadSubGrid: function (_id) {
        if (_id != null) {
            ...
            $('#GiaoThuongFnMdl-subLangMdl-List').jqGrid('setGridParam', {
                url: GiaoThuongFn.urlDefault().toString() + "&subAct=getByLangBasedId" + "&LangBased_ID=" + _id
                , loadComplete: function () {
                    adm.loading(null);
                }
            }).trigger("reloadGrid");
    ...
    filterfn: function (_active, _pub) {
        $('#GiaoThuongFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongFn.urlDefault().toString() + "&subAct=get" + "&Active=" + _active + "&Publish=" + _pub }).trigger("reloadGrid");
    },
    EditLang: function () {
        var s = '';
        if (jQuery('#GiaoThuongFnMdl-subLangMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongFnMdl-subLangMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                GiaoThuongFn.loadHtml(function () {
                    var newDlg = $('#GiaoThuongFnMdl-dlgNew');
                    GiaoThuongFn.popfn();
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 750,
                        ...
                        buttons: {
                            'Lưu': function () {
                                GiaoThuongFn.save(false, function () {
                                    GiaoThuongFn.clearform();
                                    jQuery('#GiaoThuongFnMdl-subLangMdl-List').trigger('reloadGrid');
                                });
                            },
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            $.ajax({
                                url: GiaoThuongFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
...
//                                    if (dt.Hot4) { Hot4.attr('checked', 'checked'); } else { Hot4.removeAttr('checked'); }
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                if (confirm('Bạn có chắc chắn xóa danh mục này?')) {
                    $.ajax({
                        url: GiaoThuongFn.urlDefault().toString() + '&subAct=del',
                        dataType: 'script',
                        data: { 'ID': s },
                        success: function (dt) {
                            jQuery('#GiaoThuongFnMdl-subLangMdl-List').trigger('reloadGrid');
    ...
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-GiaoThuong');
        var searchDM = $('.mdl-head-filterDanhMucGiaoThuong');
        var searchTINH = $('.mdl-head-filterKhuVucGiaoThuong');
        var _Lang = $('#GiaoThuongFnMdl-changeLangSlt').val();
        var filtermuaban = $('#GiaoThuongFnMdl-muaban');
        var filterTrangThai = $('#GiaoThuongFnMdl-TrangThai');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin') {
            q = '';
        }
        ...
        timerSearch = setTimeout(function () {
            $('#GiaoThuongFnMdl-List').jqGrid('setGridParam', { url: GiaoThuongFn.urlDefault().toString() + '&subAct=get&dm=' + dm + '&dmkv=' + tinh + '&Lang=' + _Lang + '&q=' + q + '&muaban=' + muaban + '&trangthai=' + trangthai }).trigger('reloadGrid');
        }, 500);
    },
    add: function () {
        GiaoThuongFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        GiaoThuongFn.save(false, function () {
                            GiaoThuongFn.clearform();
                            jQuery('#GiaoThuongFnMdl-List').trigger('reloadGrid');
                        });
                    },
                    'Lưu và đóng': function () {
                        GiaoThuongFn.save(false, function () {
    ...
    loadHtml: function (fn) {
        var dlg = $('#GiaoThuongFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.giaoThuong.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
    ...
    edit: function () {
        var s = '';
        if (jQuery('#GiaoThuongFnMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#GiaoThuongFnMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            alert('Chọn mẩu tin để sửa');
        }
...
//        var _Hot4 = Hot4.is(':checked');
        ...
        if (_LangBased_ID != '' && eval(_LangBased) == false) {
            $.ajax({
                url: GiaoThuongFn.urlDefault().toString() + '&subAct=CountLang',
                dataType: 'script',
                data: {
                    'ID': _LangBased_ID,
                    'Lang': _Lang
...
//                    'Hot4': _Hot4
    ...
    del: function () {
        var s = '';
        s = jQuery('#GiaoThuongFnMdl-List').jqGrid('getGridParam', 'selarrrow').toString();
    ...
    AddLang: function () {
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                var treedata = $("#GiaoThuongFnMdl-List").jqGrid('getRowData', s);
    ...
    PheDuyet: function (active) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn duyệt các mẩu tin này?')) {
                $.ajax({
                    url: GiaoThuongFn.urlDefault().toString() + '&subAct=PheDuyet' + '&active='+ active,
                    dataType: 'script',
                    data: {
                        'ID': s,
                    },
