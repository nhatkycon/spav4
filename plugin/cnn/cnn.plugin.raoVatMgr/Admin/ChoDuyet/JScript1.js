RaoVatAdminChoDuyetFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.raoVatMgr.Admin.ChoDuyet.Class1, cnn.plugin.raoVatMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        ...
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                $('#RaoVatAdminChoDuyetFnMdl-List').jqGrid({
                    url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=get&'+'&trangthai=2',
                    datatype: 'json',
                    loadui: 'disable',
                    colNames: ['ID','TT','Ảnh', 'Tiêu đề', 'Danh mục RV', 'Loại tin', 'Ngày Yêu cầu', 'Từ ngày','Đến ngày'],
                    colModel: [
                        ...
                        { name: 'RV_NgayCapNhat', width: 30, resizable: true, sortable: true, align: "center" },
                        { name: 'TuNgay', width: 30, resizable: true, sortable: true, align: "center" },
                        { name: 'DenNgay', width: 30, resizable: true, sortable: true, align: "center" }

                    ],
                    caption: 'Danh sách tin Rao vặt Đăng ký Vip',
                    autowidth: true,
                    autoheight: true,
                    ...
                    multiselect: true,
                    multiboxonly: true,
                    pager: $('#RaoVatAdminChoDuyetFnMdl-Pager'),
                    onSelectRow: function (rowid) {
                    },
                    loadComplete: function () {
                        adm.loading(null);
                        var txtfilter = $('.mdl-head-filterDanhMucRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                        var txtfilterTINH = $('.mdl-head-filterKhuVucRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                        var txtfilterLoaiTin = $('.mdl-head-filterLoaiTinRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                ...
                var filterLoaiDanhMucID = $('.mdl-head-filterDanhMucRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                var filterTINHID = $('.mdl-head-filterKhuVucRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                var filterLoaiTin = $('.mdl-head-filterLoaiTinRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                var searchTxt = $('.mdl-head-search-RaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
                ...
                var TrangThai = $('#RaoVatAdminChoDuyetFnMdl-TrangThai');
                $(TrangThai).find('option').remove();
                $(TrangThai).prepend('<option value="1">Đăng ký SUPER</option>');
                $(TrangThai).prepend('<option value="3">Đăng ký HOT</option>');
                $(TrangThai).prepend('<option value="2">Đăng ký VIP</option>');
                $(TrangThai).attr('value',2);
                //                var muaban = $('#RaoVatAdminChoDuyetFnMdl-muaban');
                ...
                var changeLangBtn = $('#RaoVatAdminChoDuyetFnMdl-changeLangSlt');
    ...
    search: function () {
        var timerSearch;
        var filterDM = $('.mdl-head-search-RaoVatAdminChoDuyet');
        var searchDM = $('.mdl-head-filterDanhMucRaoVatAdminChoDuyet');
        var searchTINH = $('.mdl-head-filterKhuVucRaoVatAdminChoDuyet');
        var searchLoaiTin = $('.mdl-head-filterLoaiTinRaoVatAdminChoDuyet', '#RaoVatAdminChoDuyetFnMdl-head');
        var _Lang = $('#RaoVatAdminChoDuyetFnMdl-changeLangSlt').val();
        var filtermuaban = $('#RaoVatAdminChoDuyetFnMdl-muaban');
        var filterTrangThai = $('#RaoVatAdminChoDuyetFnMdl-TrangThai');
        var q = filterDM.val();
        if (q == 'Tìm kiếm tin') {
            q = '';
        }
        ...
        timerSearch = setTimeout(function () {
            $('#RaoVatAdminChoDuyetFnMdl-List').jqGrid('setGridParam', { url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=get&dm=' + dm + '&dmkv=' + tinh + '&Lang=' + _Lang + '&q=' + q + '&muaban=' + loaitin + '&trangthai=' + trangthai }).trigger('reloadGrid');
        }, 500);
    },
    loadHtml: function (fn) {
        var dlg = $('#RaoVatAdminChoDuyetFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.raoVatMgr.Admin.ChoDuyet.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
    ...
    add: function () {
        RaoVatAdminChoDuyetFn.loadHtml(function () {
            var newDlg = $('#RaoVatAdminChoDuyetFnMdl-dlgNew');
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        RaoVatAdminChoDuyetFn.save(false, function () {
                            RaoVatAdminChoDuyetFn.clearform();
                            jQuery('#RaoVatAdminChoDuyetFnMdl-List').trigger('reloadGrid');
                        });
                    },
    ...
    save: function (validate, fn) {
        ...
        $.ajax({
            url: RaoVatAdminChoDuyetFn.urlDefault().toString(),
            dataType: 'script',
            type: 'POST',
            data: {
                'subAct': 'save',
    ...
    edit: function () {
        var s = '';
        if (jQuery('#RaoVatAdminChoDuyetFnMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#RaoVatAdminChoDuyetFnMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
                ...
                RaoVatAdminChoDuyetFn.loadHtml(function () {
                    ...
                    $(newDlg).dialog({
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            $.ajax({
                                url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    del: function () {
        var s = '';
        s = jQuery('#RaoVatAdminChoDuyetFnMdl-List').jqGrid('getGridParam', 'selarrrow').toString();
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {
                $.ajax({
                    url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
                        jQuery('#RaoVatAdminChoDuyetFnMdl-List').trigger('reloadGrid');
    ...
    PheDuyet: function (active) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn duyệt các mẩu tin này?')) {
                $.ajax({
                    url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=PheDuyet' + '&active=' + active,
                    dataType: 'script',
                    data: {
                        'ID': s
                    },
    ...
    DangTinDungTin: function (Publish) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc muốn đăng các mẩu tin này?')) {
                $.ajax({
                    url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=DangTinDungTin' + '&Publish='+ Publish,
                    dataType: 'script',
                    data: {
                        'ID': s,
                    },
    ...
    popfndkdv: function () {
        var newDlg = $('#RaoVatAdminChoDuyetFnMdl-DKDVFnMdl-dlgNew');
...
//        GhiChuDKDVTT.html('');
    ...
    loadHtmlDKDV: function (fn) {
        var dlg = $('#RaoVatAdminChoDuyetFnMdl-DKDVFnMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.raoVatMgr.Admin.ChoDuyet.Dkdv.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
    ...
    TinhGiaCuoc: function(NgayHetHan, NgayBatDau,Gia){
        ...
        if(parent.Date(EndDate) > parent.Date(StartDate)){
            var SoNgay = RaoVatAdminChoDuyetFn.dateDiff(d,EndDate,StartDate);
            //////console.log(SoNgay);
    ...
    DKDV: function () {
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                RaoVatAdminChoDuyetFn.loadHtmlDKDV(function () {
                    RaoVatAdminChoDuyetFn.popfndkdv();
                    var newDlg = $('#RaoVatAdminChoDuyetFnMdl-DKDVFnMdl-dlgNew');
                    $(newDlg).dialog({
                    ...
                    beforeClose: function () {
                        RaoVatAdminChoDuyetFn.clearformdkdv();
                    },
                        open: function () {
...
//                                    });
                            ...
                            //////console.log('2');
                            $.ajax({
                                url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=LoadHoTroDKDV',
                                dataType: 'script',
                                data: {
                                    'MaDanhMuc':'LIENHE'
                                },
                            ...
                            $.ajax({
                                url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=LoadThanhToanDanhMuc',
                                dataType: 'script',
                                data: {
                                },
                                success: function (dt) {
...
//                                        }
                                    ...
                                    NgayBatDauSuper.change(function(){
                                        TongTienSuper.html('');
                                        SoNgayDangTinSuperhtm.html('');
                                        var convertDateNgayHetHanSuper = RaoVatAdminChoDuyetFn.convertDate(NgayHetHanSuper.val(),'/');
                                        var convertDateNgayBatDauSuper = RaoVatAdminChoDuyetFn.convertDate(NgayBatDauSuper.val(),'/'); 
                                        var SoNgayDangKySuper = RaoVatAdminChoDuyetFn.dateDiff('d',convertDateNgayBatDauSuper,convertDateNgayHetHanSuper);
                                    ...
                                    NgayBatDauHot.change(function(){
                                        TongTienHot.html('');
                                        SoNgayDangTinHothtm.html('');
                                        var convertDateNgayHetHanHot = RaoVatAdminChoDuyetFn.convertDate(NgayHetHanHot.val(),'/');
                                        var convertDateNgayBatDauHot = RaoVatAdminChoDuyetFn.convertDate(NgayBatDauHot.val(),'/'); 
                                        var SoNgayDangKyHot = RaoVatAdminChoDuyetFn.dateDiff('d',convertDateNgayBatDauHot,convertDateNgayHetHanHot);
                                    ...
                                    NgayHetHanVip.change(function(){
                                        TongTienVip.html('');
                                        SoNgayDangTinViphtm.html('');
                                        var convertDateNgayHetHanVip = RaoVatAdminChoDuyetFn.convertDate(NgayHetHanVip.val(),'/');
                                        var convertDateNgayBatDauVip = RaoVatAdminChoDuyetFn.convertDate(NgayBatDauVip.val(),'/'); 
                                        var SoNgayDangKyVip = RaoVatAdminChoDuyetFn.dateDiff('d',convertDateNgayBatDauVip,convertDateNgayHetHanVip);
    ...
    saveDKDVVIP: function(validate, fn){
        ...
        adm.loading('Đang lưu dữ liệu');
        $.ajax({
            url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=DKDV',
            dataType: 'script',
            type: 'POST',
            data: {
                'ID': _ID,
            ...
            success: function (dt) {
                ...
                if (dt == '1') {
                    ...
                    jQuery('#RaoVatAdminChoDuyetFnMdl-List').trigger('reloadGrid');
                    //btnVip.fadeTo("slow",0.25);
                    //btnVip.removeAttr('href');
                    btnVip.hide();
                    //alert('Bạn đã đăng ký thành công dịch vụ tin rao vặt VIP.');
                    var ThongTinRaoVatTrangThaiDKDV = $('.ThongTin-RaoVat-TrangThaiDKDV',newDlg);
                    //////console.log(_ID);
                    RaoVatAdminChoDuyetFn.LoadDichVuDaDangKy(ThongTinRaoVatTrangThaiDKDV,_ID);
                }
                else {
                    spbMsg.html('Lỗi máy chủ, chưa thể lưu được dữ liệu');
                }
    ...
    sendLienHe: function (validate, fn) {
        ...
        $.ajax({
            url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=lienHe',
            dataType: 'script',
            data: {
                'ID': ID,
    ...
    LoadFormDKDVTraPhiHtml: function (fn) {
        var dlg = $('#DuyetDKRVTP-Dlg');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("cnn.plugin.raoVatMgr.Admin.ChoDuyet.DuyetDKRVTP.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
    ...
    LoadFormDKDVTraPhi: function (_grid) {
        var grid = $(_grid);
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                RaoVatAdminChoDuyetFn.LoadFormDKDVTraPhiHtml(function () {
                    var newDlg = $('#DuyetDKRVTP-Dlg');
                    $(newDlg).dialog({
                        title: 'Duyệt đăng ký dịch vụ trả phí',
                        width: 637,
                        height: 415,
                        modal: true,
                        buttons: {
                            'Duyệt và đóng': function () {
                                RaoVatAdminChoDuyetFn.SaveDKDVTraPhi(false, function () {
                                    $(newDlg).dialog('close');
                                    jQuery('#RaoVatAdminChoDuyetFnMdl-List').trigger('reloadGrid');
                                });
                            },
                        ...
                        beforeClose: function () {
                            RaoVatAdminChoDuyetFn.ClearFormDKDVTraPhi();
                        },
                        open: function () {
                            adm.styleButton();
                            RaoVatAdminChoDuyetFn.ClearFormDKDVTraPhi();
                            RaoVatAdminChoDuyetFn.popfnFormDKDVTraPhi();
                            $.ajax({
                                url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                ...
                                success: function (dt) {
                                    ...
                                    var Anh = $('.Anh', newDlg);
                                    if (dt.Anh1 != '') {
                                        $(Anh).attr('src', '../up/i/' + dt.Anh1 + '?ref=' + Math.random());
                                    }
                                    var Ten = $('.Ten', newDlg);
                                    Ten.append(data.Ten);
                                    ...
                                    var member = eval(data._Member)
                                    NguoiDaiDien.append(member.Ten);
                                    var DienThoai = $('.DienThoai', newDlg);
                                    DienThoai.append(member.Mobile + ' ' + member.Phone);
                                    var Email = $('.Email', newDlg);
                                    Email.append(member.Email);
                                    var NgayDangKy = $('.NgayDangKy', newDlg);
                                    ...
                                    var cbkDKRVVip = $('.cbkDKRVVip', newDlg);
                                    var NgayKTDKRVVIP = $('.NgayKTDKRVVIP', newDlg);
                                    var NgayDKRVVIP = $('.NgayDKRVVIP', newDlg);
                                    
                                    if(eval(data.DangKy_VIP)== true){
                                        cbkDKRVVip.attr('checked','checked');
                                        var __ngayDKHH = new Date(data.VIP_VIP_NgayBatDau);
                                        var _ngayDKHH = __ngayDKHH.getDate() + '/' + (__ngayDKHH.getMonth() + 1) + '/' + __ngayDKHH.getFullYear();
                                        NgayDKRVVIP.val(_ngayDKHH);
                                        var __ngayDK = new Date(data.VIP_VIP_NgayHetHan);
                                        var _ngayDK = __ngayDK.getDate() + '/' + (__ngayDK.getMonth() + 1) + '/' + __ngayDK.getFullYear();
                                        NgayKTDKRVVIP.val(_ngayDK);
                                    }
                                    var cbkRVTT = $('.cbkRVTT', newDlg);

                                    cbkRVTT.unbind('click').click(function(){
                                        if(eval(cbkRVTT.is(':checked')) == true){
                                            cbkDKRVVip.attr('disabled','disabled');
                                            cbkDKRVVip.removeAttr('checked');
                                            NgayDKRVVIP.val('');
                                            NgayKTDKRVVIP.val('');
                                            NgayDKRVVIP.attr('disabled','disabled');
                                            NgayKTDKRVVIP.attr('disabled','disabled');
                                        }
                                        if(eval(cbkRVTT.is(':checked')) == false){
                                            cbkDKRVVip.removeAttr('disabled');
                                            NgayDKRVVIP.removeAttr('disabled');
                                            NgayKTDKRVVIP.removeAttr('disabled');
                                        }
                                    });
                                }
    ...
    SaveDKDVTraPhi: function(validate,fn){
        var vip = '';
        var newDlg = $('#DuyetDKRVTP-Dlg');
        var ID = $('.ID', newDlg);
        var cbkDKRVVip = $('.cbkDKRVVip', newDlg);
        ...
        var spbMsg = $('.admMsg', newDlg);
        var _ID = ID.val();
        var cbkvip = eval(cbkDKRVVip.is(':checked'));
        var cbkTT = eval(cbkRVTT.is(':checked'));
        //console.log(cbkTT);
        var NgayBD = NgayDKRVVIP.val();
        var NgayKT = NgayKTDKRVVIP.val();

        var err = '';
        if(cbkvip == true){
            var convertDateNgayHetHanSuper = RaoVatAdminChoDuyetFn.convertDate(NgayKT,'/');
            var convertDateNgayBatDauSuper = RaoVatAdminChoDuyetFn.convertDate(NgayBD,'/');
            if(Date.parse(convertDateNgayHetHanSuper) < Date.parse(convertDateNgayBatDauSuper)){
                err += 'ngày kết thúc phải lớn hơn ngày đăng ký';
            }
            if(NgayBD ==''){
            ...
            if (NgayBD != '' && NgayKT!= '' && (Date.parse(convertDateNgayHetHanSuper) >= Date.parse(convertDateNgayBatDauSuper))) {
                vip = 'True';
            }
        }
        if(cbkTT == true){
        ...
        adm.loading('Đang lưu dữ liệu');
        $.ajax({
            url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=DuyetDKDVRVTP',
            dataType: 'script',
            type: 'POST',
            data: {
                'ID': _ID,
    ...
    ClearFormDKDVTraPhi: function(){
        ...
        NgayDKRVVIP.val('');
        NgayDKRVVIP.removeAttr('disabled');
        cbkRVTT.removeAttr('checked');
        cbkRVTT.removeAttr('disabled');
    },
    popfnFormDKDVTraPhi: function(){
        var newDlg = $('#DuyetDKRVTP-Dlg');
        var NgayKTDKRVVIP = $('.NgayKTDKRVVIP', newDlg);
        var NgayDKRVVIP = $('.NgayDKRVVIP', newDlg);
        $(NgayKTDKRVVIP).datepicker({ defaultDate: +7, dateFormat: 'dd/mm/yy', showButtonPanel: true });
        $(NgayDKRVVIP).datepicker({ defaultDate: +7, dateFormat: 'dd/mm/yy', showButtonPanel: true });
        
    },
    QuickSaveDichVu: function(_vip,_grid){
        var grid = $(_grid);
        var s = '';
        if (jQuery(grid).jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery(grid).jqGrid('getGridParam', 'selrow').toString();
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                $.ajax({
                    url: RaoVatAdminChoDuyetFn.urlDefault().toString() + '&subAct=QuickSaveDichVu',
                    dataType: 'script',
                    type: 'POST',
                    data: {
                        'ID': s,
