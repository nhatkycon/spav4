var quangcaofn = {
    urlDefault: adm.urlDefault + '&act=loadPlug&rqPlug=cnn.plugin.quangCao.daDuyet.Class1, cnn.plugin.quangCao',
    setup: function () {
    },
    loadgrid: function () {
    ...
    loadHtml: function (fn) {
        ...
        if ($(dlg).length < 1) {
            ...
            $.ajax({
                url: '<%=WebResource("cnn.plugin.quangCao.daDuyet.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
