var tinFn = {
    urlDefault: adm.urlDefault + '&act=loadPlug&rqPlug=plugin.rss.KTNN.Class1, plugin.rss.KTNN',
    setup: function () {
    },
    loadgrid: function () {
        adm.loading('Đang lấy dữ liệu');
        adm.styleButton();
        $('#tinMdl-List-ktnn').jqGrid({
            ...
            pager: jQuery('#tinMdl-Pager-ktnn'),
           onSelectRow: function (rowid) {
            ...
            loadComplete: function () {
               ...
               // var ID = $('.ID', newDlg);
              //  var _ID = ID.val();
                var txtfilter = $('.mdl-head-tinFilterByBao-ktnn');
                ...
                //var searchTxt = $('.mdl-head-search-tinMdl'); adm.regQS(searchTxt, 'tinMdl-List-ktnn');
                // tinFn.headFn();
                //  tinFn.loadTinList('0');
    ...
    headFn: function () {
        var filterByBao = $('.mdl-head-tinFilterByBao-ktnn');
    ...
    search: function () {
        ...
        timerSearch = setTimeout(function () {
            $('#tinMdl-List-ktnn').jqGrid('setGridParam', { url: tinFn.urlDefault
            + '&subAct=get'
            + '&Bao=' + _B_ID
            + '&DM_ID=' + _DM_ID
            + '&TpId=' + _T_ID
    ...
    savetintuc: function () {
        ...
        s = jQuery("#tinMdl-List-ktnn").jqGrid('getGridParam', 'selarrrow').toString();
        if (s == '') {
            alert('Chọn mẩu tin để lưu');
        }
        else {
           // if (confirm('Bạn có chắc chắn xóa tin này?')) {
                ...
                for (j = 0; j < ss.length; j++) {
                    var treedata = $("#tinMdl-List-ktnn").jqGrid('getRowData', ss[j]);
                    ll += treedata.T_ID + ',';
                }
            //}
                {
                    var txtfilter = $('.mdl-head-tinFilterByBao-ktnn');
                    var dm = $(txtfilter).attr('_value');
                    alert(ll + 'danh muc' + dm);
                $.ajax({
                  url: tinFn.urlDefault + '&subAct=saveTintuc',
                        dataType: 'script',
                        type: 'POST',
                        data: {
                         'ID': ll,
                        ...
                        if (_dt == '1') {
                            ...
                            jQuery('#tinMdl-List-ktnn').trigger('reloadGrid');
                            //   var treedata = $("#tinMdl-List-ktnn").jqGrid('getRowData', rowid);
    ...
    edit: function () {
        var s = '';
        if (jQuery('#tinMdl-List-ktnn').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#tinMdl-List-ktnn').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
    ...
    loadsubfunction: function (role_id) {
        ...
        var treeDiv = $('#laytintudongMdl-ktnn-functionmdl-roleFnMdl');
        $.ajax({
            ...
            data: {
                ...
                if (data == '0') {
                    ...
                    $.each($('a', '#laytintudongMdl-ktnn-functionmdl-roleFnMdl'), function (i, item) {
    ...
    loadHtml: function (fn) {
        ...
        if ($(dlg).length < 1) {
            ...
            $.ajax({
                url: '<%=WebResource("plugin.rss.KTNN.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
