var danhmuc = {
            ...
            languageFn.setup(function () {
                $('#danhmucmdl-List').jqGrid({
                    ...
                    colModel: [
                        ...
                        { name: 'DM_Lang', width: 30, sortable: true, align: "center" },
                        { name: 'DM_ThuTu', width: 20, sortable: true, align: "center" },
                        { name: 'LDM_Ten', width: 10, resizable: true, sortable: true },
                        { name: 'DM_Ma', width: 40, resizable: true, sortable: true, align: "center" },
                        { name: 'DM_KyHieu', width: 40, resizable: true, sortable: true, align: "center" },
                        { name: 'DM_GiaTri', width: 40, resizable: true, sortable: true },
                        { name: 'DM_Anh', width: 10, resizable: true, sortable: true },
                        { name: 'DM_Ten', width: 100, resizable: true, sortable: true },
                        { name: 'DM_NgayCapNhat', width: 40, resizable: true, sortable: true, align: "center" },
                        { name: 'DM_NguoiTao ', width: 40, resizable: false, sortable: true, align: "center" }
                    ],
                    caption: 'Danh sách',
                    autowidth: true,
                    sortname: 'DM_ThuTu',
    ...
    ,
    autoCompleteByLoai: function (s, el, fn) {
        $(el).autocomplete({
            source: function (request, response) {
    ...
    autoCompleteGianHangByUsername: function (id, ldm_ma, el, fn) {
        $(el).autocomplete({
            source: function (request, response) {
                var term = 'danhMuc-autoCompleteLangBased' + id + ldm_ma;
                adm.loading('Nạp danh sách');
    ...
    getSelectionData: function (ldm_ma, fn, _objmain) {
        var objmain = $(_objmain);
        var term = 'danhMuc-getSelectTion' + ldm_ma + objmain;
        if (!(term in _cache)) {
            $.ajax({
                url: danhmuc.urlDefault().toString(),
                dataType: 'script',
                data: { 'subAct': 'autoCompleteLangBased', 'LDM_Ma': ldm_ma },
                success: function (_dt) {
                    _cache[term] = eval(_dt);
                    fn(_cache[term]);
                }
            });
        }
    ...
    getSelectTionTest: function (ldm_ma, _objintput, _objoutput, _objmain) {
        var objmain = $(_objmain);
        _objoutput.addClass('addclassDanhMucBox_show');
        danhmuc.getSelectionData(ldm_ma, function (dt) {
            var term = 'danhMuc-getSelectTion' + ldm_ma + objmain;
            var _objIntput = $(_objintput);
            var _objOutput = $(_objoutput);
            var myDt = _cache[term];
            if ($(objmain).find('#productCategoryListContainer').length < 1) {
                $('<div id="productCategoryListContainer"></div>').appendTo(_objOutput);
                var productCategoryListContainer = $('#productCategoryListContainer', objmain);
                //$('<h4 class="selectedLastInfo" style="height:15px;"></h4>').appendTo(productCategoryListContainer);
                //$('<div style="height:15px;margin:10px;"><table class="selectedLastInfo"><tr><th></th></tr></table></div>').appendTo(productCategoryListContainer);
                $('<div style="height:30px;"><div class="selectedLastInfo" style="float:left;"><span>Lưu danh mục sản phẩm đã chọn</span><input id="hiddenProductListOption" type="button" value="Chọn"/></div><input id="hiddenProductListOption-btnHuy" type="button" value="Hủy"/></div>').appendTo(productCategoryListContainer);
                $('<div id="danhMuc-geSelectTion-Mdl"></div>').appendTo(productCategoryListContainer);
                var divlistproduct = $('#danhMuc-geSelectTion-Mdl', objmain);
                $('<div class="danhMuc-geSelectTion-Mdl-1 danhMuc-geSelectTion-Mdl"></div>').appendTo(divlistproduct);
                $('<div class="danhMuc-geSelectTion-Mdl-2 danhMuc-geSelectTion-Mdl"></div>').appendTo(divlistproduct);
                $('<div class="danhMuc-geSelectTion-Mdl-3 danhMuc-geSelectTion-Mdl"></div>').appendTo(divlistproduct);
            }
            var btnHuy = $('#hiddenProductListOption-btnHuy', objmain);
            ////console.log(btnHuy);
            ...
            var productCategoryListContainer = $('#productCategoryListContainer', objmain);
            var divlistproduct = $('#danhMuc-geSelectTion-Mdl', objmain);
            var div1 = $('.danhMuc-geSelectTion-Mdl-1', objmain);
            var div2 = $('.danhMuc-geSelectTion-Mdl-2', objmain);
            var div3 = $('.danhMuc-geSelectTion-Mdl-3', objmain);
            var divbtn = $('.selectedLastInfo', objmain);
            $(div1).show();
            $(div2).hide();
            $(div3).hide();
            divbtn.hide();
            ...
            $.each(myDt, function (i, item) {
                if (item.Level == 1) {
                    var _item = $('<div class="SubdanhMuc-geSelectTion-Mdl" _text="' + item.Ten + '" _id="' + item.ID + '">' + item.Ten + '</div>').appendTo(div1);
                    if ($(_objIntput).find('.spanlv1').length == 1) {
                        var slv1 = $(_objIntput).find('.spanlv1').attr('_id');
                    ...
                    _item.click(function () {
                        ...
                        if ($('[_pid="' + _pid + '"]', div2).length == 0) {
                            ...
                            _item.dblclick(function () {
                                ...
                                $('<span class="spanlv1" level="1" _id="' + spanidlv1 + '">' + spannamelv1 + '</span>').appendTo(_objIntput);
                                $(_objIntput).attr('value', spanidlv1);
                            });
            ...
            $.each(myDt, function (i, item) {
                if (item.Level == 2) {
                    ...
                    if ($(_objIntput).find('.spanlv2').length == 1) {
                        var slv1 = $(_objIntput).find('.spanlv1').attr('_id');
                        $('[_id="' + slv1 + '"]', div1).addClass('SubDanhmuc-item-selected');
                        $(div2).show();
                        $('[_pid="' + slv1 + '"]', div2).show();
                        var slv2 = $(_objIntput).find('.spanlv2').attr('_id');
                    ...
                    _item.click(function () {
                        ...
                        if ($('[_pid="' + _pid + '"]', div3).length == 0) {
                            ...
                            _item.dblclick(function () {
                                ...
                                var spannamelv1 = $(div1).find('.SubDanhmuc-item-selected').attr('_text');
                                $('<span class="spanlv1" level="1" _id="' + spanidlv1 + '">' + spannamelv1 + '</span>').appendTo(_objIntput);
                                $('<span> >> </span>').appendTo(_objIntput);
                                var spanidlv2 = $(div2).find('.SubDanhmuc-item-selected').attr('_id');
                                var spannamelv2 = $(div2).find('.SubDanhmuc-item-selected').attr('_text');
                                $('<span class="spanlv2" level="2" _id="' + spanidlv2 + '">' + spannamelv2 + '</span>').appendTo(_objIntput);
                                $(_objIntput).attr('value', spanidlv2);
                            });
                            divbtn.click(function () {
                                $(_objOutput).hide();
                                $(_objIntput).html('');
            ...
            $.each(myDt, function (i, item) {
                if (item.Level == 3) {
                    ...
                    if ($(_objIntput).find('.spanlv3').length == 1) {
                        var slv1 = $(_objIntput).find('.spanlv1').attr('_id');
                        $('[_id="' + slv1 + '"]', div1).addClass('SubDanhmuc-item-selected');
                        $(div2).show();
                        $('[_pid="' + slv1 + '"]', div2).show();
                        ...
                        $(div3).show();
                        $('[_pid="' + slv2 + '"]', div3).show();
                        var slv3 = $(_objIntput).find('.spanlv3').attr('_id');
                    ...
                    _item.click(function () {
                        ...
                        _item.dblclick(function () {
                            ...
                            $('<span class="spanlv3" level="3" _id="' + spanidlv3 + '">' + spannamelv3 + '</span>').appendTo(_objIntput);
                            $(_objIntput).attr('value', spanidlv3);
                        });
                        divbtn.click(function () {
                            $(_objOutput).hide();
                            $(_objIntput).html('');
    ...
    autoCompleteLangBasedByMaDanhMuc: function (Ma, el, fn) {
        //////console.log(id);
        $(el).autocomplete({
            source: function (request, response) {
                var term = 'danhMuc-autoCompleteLangBasedByMaDanhMuc' + Ma;
                adm.loading('Nạp danh sách');
                _lastXhr = $.ajax({
                    url: danhmuc.urlDefault().toString(),
                    dataType: 'script',
                    data: { 'subAct': 'autoCompleteLangBasedByMaDanhMuc', 'Ma': Ma },
                    success: function (dt, status, xhr) {
                        adm.loading(null);
                        var data = eval(dt);
                        _cache[term] = data;
    ...
    autoCompleteObj: function (el, availableTags, fn) {
        $(el).autocomplete({
