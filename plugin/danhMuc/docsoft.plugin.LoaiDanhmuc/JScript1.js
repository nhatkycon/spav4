var loaidanhmuc = {
    ...
    loaddanhmuc: function (DM_ID, DM_Ten) {
        adm.regType(typeof (languageFn), 'cnn.plugin.language.Class1, cnn.plugin.language', function () {
            languageFn.setup(function () {
                // Nạp lựa chọn ngôn ngữ
                var changeLangBtn = $('#loaidanhmucmdl-danhMucMdl-changeLangSlt');
                ...
                // End Nạp lựa chọn ngôn ngữ
                //#region Nạp lưới con
                $('#subdanhmucmdl-List').jqGrid({
                    url: danhmuc.urlDefault().toString() + '&subAct=get&Lang=' + Lang + '&LDMID=' + DM_ID,
                    height: '400',
                    datatype: 'json',
                    colNames: ['ID', 'LangBased', '_ID', 'Lang', 'Thứ tự', 'Loại Danh Mục', 'Mã', 'Ký Hiệu', 'Giá Trị', 'Tên', 'Ngày cập nhật', 'Người tạo/sửa'],
                    colModel: [
                        ...
                        { name: 'LDM_Ten', width: 80, resizable: true, sortable: true },
                        { name: 'DM_Ma', width: 40, resizable: true, sortable: true, align: "left" },
    ...
    editdanhmuc: function () {
        adm.regType(typeof (danhmuc), 'docsoft.plugin.danhmuc.Class1,docsoft.plugin.danhmuc', function () {
            danhmuc.edit('#subdanhmucmdl-List');
        });
    },
    deletedanhmuc: function () {
        adm.regType(typeof (danhmuc), 'docsoft.plugin.danhmuc.Class1,docsoft.plugin.danhmuc', function () {
            danhmuc.del('#subdanhmucmdl-List');
        });
    },
    add: function () {
    ...
    setAutocomplete: function (el, fn) {
        $(el).autocomplete({
            source: function (request, response) {
                ...
                $.ajax({
                    url: loaidanhmuc.urlDefault + '&subAct=getPid',
                    dataType: 'script',
                    data: {
                        'q': request.term
                    },
