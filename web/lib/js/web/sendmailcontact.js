var sendmailcontactFn = {
    usersendmailcontactFn: function () {
        ...
        var Mail = jQuery('.Email', Contact);
        var Dienthoai = jQuery('.DienThoai', Contact);
        ...
        Mail.keyup(function () {
            ...
            if (!adm.verifyEmail(_val)) { item.next().attr('_error', '1'); }
            else { item.next().attr('_error', '0'); }
        ...
        btnSend.click(function () {
            jQuery.ajax({
                url: adm.urlDefault,
                type: 'POST',
                data: { 'act': 'loadPlug', 'rqPlug': 'cnn.plugin.DangKyDichVu.LienHeBanQuanTri.Class1, cnn.plugin.DangKyDichVu', 'subAct': 'sendmailContact', NguoiGui: NguoiGui.val(), msgtitle: TieuDe.val(), NoiDungLienHe: NoiDung.val(), Email: Mail.val(), DienThoai: Dienthoai.val() },
                success: function (_dt) {
                    var Contact = jQuery('#user-contact');
                    var spInfo = jQuery('.admInfo', Contact);
                    if (_dt == '1') {
                        alert('Bạn đã gửi thành công!');
                        sendmailcontactFn.clearText();
                    }
                   else {
    ...
    clearText: function () {
        ...
        jQuery('.DienThoai', Contact).val('');
        jQuery('.Email', Contact).val('');
