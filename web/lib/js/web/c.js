var common = {
    ...
    createFck: function (el) {
        var config = {
            toolbar:
		[
			['Bold', 'Italic', '-', 'NumberedList', 'BulletedList', '-', 'Link', 'Unlink', 'Image', 'Flash', 'Smiley'],
        ...
        var editor = jQuery(el).ckeditor(config, function () {
            CKFinder.setupCKEditor(this, domain + '/lib/js/ckfinder/');
        });
    },
    getQuery: function () {
    ...
    regType: function (_t, src, fn) {
        if (_t == 'undefined') {
            ...
            $.getScript(common._domain + '/' + src, function () {
                if (typeof (fn) == 'function') {
                    fn();
                }
            });
    ...
    upload: function (uploadBtn, type, param, callback) {
        common.regType(typeof (Ajax_upload), 'lib/jQuery/ajaxupload.js', function () {
            var defaultParam = { 'act': 'upload', 'subAct': type };
            param = $.extend(defaultParam, param);
            new Ajax_upload($(uploadBtn), {
                action: common._domain + '/lib/aspx/sys/up-anh.aspx?ref=' + Math.random(),
                name: 'anh',
                data: param,
