<%@ Control Language="C#" AutoEventWireup="true" CodeFile="spa_view.ascx.cs" Inherits="lib_ui_web_spa_spa_view" %>
...
<%=txtPath%>
    ...
    <meta property="og:description" content="<%=Item.DiaChi + " " + Item.Phone %>" />
    <% if (!string.IsNullOrEmpty(Item.Anh))
       { %>
    <meta property="og:image" content="/lib/up/i/<%=Lib.imgSize(Item.Anh,"150x115") %>" />
    <% }
       else
       { %>
    <meta property="og:image" content="/lib/css/web/1/i/logo.png" />
    <% } %>
...
<h3 class="spa-view-lienQuanList-header">
    NHẬN XÉT TỪ KHÁCH HÀNG (<%=Comments.Count.ToString() %>)
</h3>
...
<div style="position: absolute; top: -10px; height: 10px;overflow: hidden;">
  <div itemscope itemtype="http://data-vocabulary.org/Review">
    <span itemprop="itemreviewed"><%=Item.Ten %></span>
    Reviewed by <span itemprop="reviewer">Tạp chí Spa</span> on
    <time itemprop="dtreviewed" datetime="<%=Item.NgayTao.ToString("yyyy-MM-dd") %>">Jan 6</time>.
    <span itemprop="summary"><%=Item.DiaChi + " " + Item.Phone%></span>
    <span itemprop="description"><%=Lib.Rutgon(Item.NoiDung,200)%></span>
    Rating: <span itemprop="rating"><%=Item.Sao %></span>
  </div>
</div>